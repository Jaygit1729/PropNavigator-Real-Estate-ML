...
def main():
    ...
    try:
        ...
        # 2. Clean Indepedent Builder Floor Data

        logger.info("Step 2:Cleaning Independent Builder Floor data...")
        clean_builder = clean_builder_data("data/web_scraping/builder_floor_gurgaon.csv")
        if clean_builder_data is None:
            logger.error("Independent Builder Floor data cleaning failed (returned None).")
            return
        save_data(clean_builder, "data/data_cleaning/cleaned_independent_builder_floor.csv")
        logger.info("Independent Builder Floor data cleaned and saved.")
        
        # 3 Clean House Data

        logger.info("Step 3:Cleaning Independent House data...")
        cleaned_houses = clean_house_data("data/web_scraping/independent_house_gurgaon.csv")
        ...
        fs_df = select_features(processed_df)
        if fs_df is None:
