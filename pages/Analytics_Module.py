...
# Single clean analytics base produced by notebooks/analytics_module/analytics_module.ipynb.
# Same clean population as the model, but retains viz columns (lat/long, ppsf, features).
DATA_PATH = os.path.join(BASE_DIR, "data", "analytics_module", "analytics_base.csv")

# Minimum listings a sector must have to appear in price dispersion heatmap
MIN_LISTINGS_FOR_DISPERSION = 15

# Area segment bin edges and labels for Section 5 (area is super-built-up sqft)
...
def parse_features(value):
    """Parse the raw `features` string into a list of amenity strings."""
    if pd.isna(value):
        return []
    try:
        parsed = ast.literal_eval(value)
        return [str(a).strip() for a in parsed] if isinstance(parsed, list) else []
    except (ValueError, SyntaxError):
        return []


def extract_amenity_words(df: pd.DataFrame, sector: str):
    """
    ...
    words = []
    for lst in filtered["features"].dropna().apply(parse_features):
        words.extend(lst)
    return words


def get_sector_options(df: pd.DataFrame):
    """Returns sorted list of sectors with 'All' as first option."""
    return ["All"] + sorted(df["sector"].dropna().unique().tolist())
...
bhk_df = df[df["bedRoom"].between(1, 6)]

bhk_matrix = (
    bhk_df
    .groupby(["property_type", "bedRoom"])
    .size()
...
fig_bhk = px.imshow(
    ...
    labels={"color": "% of Listings", "x": "Bedrooms", "y": "Property Type"}
)
...
fig_violin = px.violin(
    ...
    labels={"property_type": "Property Type", "price_in_cr": "Price (Cr)"}
)
...
fig_area = px.box(
    ...
    color="area_segment",
    color_discrete_sequence=px.colors.qualitative.Set2,
    labels={"area_segment": "Area Segment", "price_per_sqft": "Price per Sqft (₹)"}
)
...
geo_df = (
    ...
    .mean(numeric_only=True)[
        ["sector", "price_per_sqft", "area", "latitude", "longitude"]
    ]
)
...
words = extract_amenity_words(df, sector_wc)
...
box_df = box_df[box_df["bedRoom"].between(1, 4)]
...
pie_df = df if sector_pie == "All" else df[df["sector"] == sector_pie]
pie_df = pie_df[pie_df["bedRoom"].between(1, 6)]
